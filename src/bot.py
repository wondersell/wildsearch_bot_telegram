...
def wb_catalog(update: Update, context: CallbackContext):
    ...
    if user.can_send_more_catalog_requests():
        ...
        context.bot.send_message(
            chat_id=user.chat_id,
            text=f'💫 Ваш лимит запросов закончился. Чтобы продолжить работу, напишите нам в чат поддержки с запросом на снятие ограничения, либо дождитесь восстановления лимита. {user.daily_catalog_requests_limit} новых запросов будут доступны через {dt.hour} часов {dt.minute} минут',
        )
